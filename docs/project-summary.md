...
- Designed a flash-sale order path using short-lived seckill tokens, Redis Lua atomic stock pre-deduct, duplicate buyer guard, server-side price validation, and outbox-backed RocketMQ async order creation.
...
4. RocketMQ decouples request admission from order creation, while the local outbox keeps accepted requests durable when MQ publishing is slow or temporarily unavailable.
